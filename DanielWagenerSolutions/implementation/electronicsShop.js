...
function getMoneySpent(keyboards, drives, b) {
  // Return early if Monica can't buy nothin'
  if (Math.min(...keyboards) >= b || Math.min(...drives) >= b) {
    return -1;
  }

  let expensiveCombinations = [];

  keyboards = keyboards.sort((val1, val2) => val2 - val1);
  drives = drives.sort((val1, val2) => val2 - val1);

  for (let i = 0; i < keyboards.length; i++) {
    for (let j = 0; j < drives.length; j++) {
      expensiveCombinations.push(keyboards[i] + drives[j]);
    }
  }

  return expensiveCombinations
